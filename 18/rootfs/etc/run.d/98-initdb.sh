...
[ -n "$POSTGRES_INITDB_BUILTIN_LOCALE" ] && set -- "$@" --builtin-locale="$POSTGRES_INITDB_BUILTIN_LOCALE"
...
[ "$POSTGRES_INITDB_NO_SYNC_DATA_FILES" = 1 ] && set -- "$@" --no-sync-data-files
[ "$POSTGRES_INITDB_NO_INSTRUCTIONS" = 1 ] && set -- "$@" --no-instructions
[ -n "$POSTGRES_INITDB_SYNC_METHOD" ] && set -- "$@" --sync-method "$POSTGRES_INITDB_SYNC_METHOD"
